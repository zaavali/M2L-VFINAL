...
const Cart = () => {
  ...
  const handleDelete = async (puid) => {
    try {
      removeFromCart(puid);
      await axios.put(`http://192.168.1.420:4000/api/prod/produit/${puid}/increment`);
  ...
  const envoyerCommandeBackend = async (commande) => {
    try {
      ...
      const response = await axios.post('http://192.168.1.420:4000/api/commande/valider', commande, {
        headers: {
  ...
  <div key={produit.puid}>
    <div className="cartitems-format cartitems-format-main">
      <p><img className="product-image" src={`http://192.168.1.420:4000/${produit.img}`} alt={produit.nom} /></p>
      <p>{produit.nom}</p>
      <p>{produit.prix} €</p>
